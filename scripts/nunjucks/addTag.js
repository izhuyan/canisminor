...
addTag.prototype.register = function (env, name, fn, options) {
	...
	env.addExtension(name, tag);
};
const placeholder = '\uFFFC';
const rPlaceholder = /(?:<|&lt;)\!--\uFFFC(\d+)--(?:>|&gt;)/g;
addTag.prototype.render = function (str, options, callback) {
...
NunjucksAsyncBlock.prototype.run = function (context, args, body, callback) {
	return this._run(context, args, trimBody(body)).then((result) => {
		callback(null, result);
	}, callback);
};
